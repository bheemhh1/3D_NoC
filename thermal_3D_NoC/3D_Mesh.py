...
for z in range(layers):
	...
	fplan = open('new_core_layer'+str(z)+'.flp','w')
	if(z!=layers-1):
		timtsv = open('new_timtsv'+str(z)+'.flp','w')
	for y in range(rows):
		for x in range(cores):
			...
			fplan.write('Router_'+str(x+cores*y+rows*cores*z)+'\t0.001100\t0.001100\t'+str(0.005100+x*0.006200)+'\t'+str(0.000180+y*0.007480)+'\n')
			fplan.write('TSV_'+str(x+cores*y+rows*cores*z)+'\t0.001100\t0.000180\t'+str(0.005100+x*0.006200)+'\t'+str(y*0.007480)+'\n')
			fplan.write('Fill_'+str(x+cores*y+rows*cores*z)+'\t0.005100\t0.001280\t'+str(x*0.006200)+'\t'+str(y*0.007480)+'\n')
			if(z!=layers-1):
				timtsv.write('TIM_unit_'+str(x+cores*y+rows*cores*z)+'\t0.006200\t0.007300\t'+str(x*0.006200)+'\t'+str(0.000180+y*0.007480)+'\n')
				timtsv.write('TIM_tsv_'+str(x+cores*y+rows*cores*z)+'\t0.001100\t0.000180\t'+str(0.005100+x*0.006200)+'\t'+str(y*0.007480)+'\n')
				timtsv.write('Filler_'+str(x+cores*y+rows*cores*z)+'\t0.005100\t0.000180\t'+str(x*0.006200)+'\t'+str(y*0.007480)+'\n')	
	fplan.close()
	if(z!=layers-1):
		timtsv.close()	

tim = open('new_TIM_layer.flp','w')
...
for z in range(layers):
	for y in range(rows):
		for x in range(cores):
			...
			elements['Fill_'+str(x+cores*y+rows*cores*z)] =	[0]*100	
				
		
pd.DataFrame(elements).to_csv('new_core3D.ptrace',sep='\t',index=False)
