...
Bft3::Bft3( const Configuration& config, const string & name )
: Network ( config, name )
{
  _ComputeSize( config );
  _Alloc( );
...
void Bft3::_ComputeSize( const Configuration& config )
{
...
void Bft3::RegisterRoutingFunctions(){

}

void Bft3::_BuildNet( const Configuration& config )
{
cout<<"BFat Tree"<<endl;
...
int l0=0;
  ...
  for ( pos = 0; pos < nPosl1; ++pos ) {
    for ( int port = 0; port < _k; ++port ) {
      int L=18; //because remaining links 1 to 2 have short lenght
      pp = pos;
      pc = _k * ( pos / 2 ) + port;
  ...
  for ( pos  = 0; pos < nPosl0; ++pos ) {
    for ( int port = 0; port < nPosl1; ++port ) {
      ...
      pc = port;
      int L=1;
if( ((pp==0) && (pc==6)) || ((pp==1)  && (pc==7)) || ((pp==2 ) && (pc==0)) || ((pp==3 ) && ( pc==1)))
      {                                                                                     
	L=2;
      }
      else
...
Router*& Bft3::_Router( int height, int pos )
{
  assert( height < _n );
  ...
  /*
int Bft3::_WireLatency( int height1, int pos1, int height2, int pos2 )
{
