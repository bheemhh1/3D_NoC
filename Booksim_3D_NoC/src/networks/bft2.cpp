...
Bft2::Bft2( const Configuration& config, const string & name )
: Network ( config, name )
{
  _ComputeSize( config );
  _Alloc( );
...
void Bft2::_ComputeSize( const Configuration& config )
{
...
void Bft2::RegisterRoutingFunctions(){

}

void Bft2::_BuildNet( const Configuration& config )
{
cout<<"BFat Tree"<<endl;
...
int l0=0;
  ...
  for ( pos = 0; pos < nPosl1; ++pos ) {
    for ( int port = 0; port < _k; ++port ) {
      int L=1; //because remaining links 1 to 2 have short lenght
      pp = pos;
      pc = _k * ( pos / 2 ) + port;
...
Router*& Bft2::_Router( int height, int pos )
{
  assert( height < _n );
  ...
  /*
int Bft2::_WireLatency( int height1, int pos1, int height2, int pos2 )
{
