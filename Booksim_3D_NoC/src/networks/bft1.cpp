...
Bft1::Bft1( const Configuration& config, const string & name )
: Network ( config, name )
{
  _ComputeSize( config );
  _Alloc( );
...
void Bft1::_ComputeSize( const Configuration& config )
{
...
void Bft1::RegisterRoutingFunctions(){

}

void Bft1::_BuildNet( const Configuration& config )
{
cout<<"BFat Tree"<<endl;
...
int l0=0;
  ...
  for ( pos = 0; pos < nPosl1; ++pos ) {
    for ( int port = 0; port < _k; ++port ) {
      int L=21; //because remaining links 1 to 2 have short lenght
      pp = pos;
      pc = _k * ( pos / 2 ) + port;
  ...
  for ( pos  = 0; pos < nPosl0; ++pos ) {
    for ( int port = 0; port < nPosl1; ++port ) {
      ...
      pc = port;
      int L=1;
 if((pp==0 && pc==0) || (pp==2 &&pc==4) || (pp==3 && pc==7 ))
      {
	L=92;
      }
      else
...
Router*& Bft1::_Router( int height, int pos )
{
  assert( height < _n );
  ...
  /*
int Bft1::_WireLatency( int height1, int pos1, int height2, int pos2 )
{
