...
Bft211::Bft211( const Configuration& config, const string & name )
: Network ( config, name )
{
  _ComputeSize( config );
  _Alloc( );
...
void Bft211::_ComputeSize( const Configuration& config )
{
...
void Bft211::RegisterRoutingFunctions(){

}

void Bft211::_BuildNet( const Configuration& config )
{
cout<<"BFat Tree"<<endl;
...
int l0=0;
  ...
  // Connections between h = 0 and h = 1 Levels
    ...
    _chan_cred[c]->SetLatency( L );
    c++;
         cout << "connecting (0,"<<0<<") <-> (1,"<<2<<")"<<"With Latency="<<L<<"clcok cycle"<<endl;
	_Router( 0, 0)->AddOutputChannel( _chan[c], _chan_cred[c] );	
    ...
    c++;
     ...
     cout << "connecting (0,"<<1<<") <-> (1,"<<1<<")"<<"With Latency="<<L<<"clcok cycle"<<endl;
    ...
    _chan_cred[c]->SetLatency( L );
    c++;
     cout << "connecting (0,"<<2<<") <-> (1,"<<6<<")"<<"With Latency="<<L<<"clcok cycle"<<endl;
	_Router( 0, 2)->AddOutputChannel( _chan[c], _chan_cred[c] );	
    ...
    _chan_cred[c]->SetLatency( L );
    c++;
cout << "connecting (0,"<<3<<") <-> (1,"<<5<<")"<<"With Latency="<<L<<"clcok cycle"<<endl;
...
Router*& Bft211::_Router( int height, int pos )
{
  assert( height < _n );
  ...
  /*
int Bft211::_WireLatency( int height1, int pos1, int height2, int pos2 )
{
