...
//the file neme is changed to solev a name conflict with booksim 
...
\param int model
\return  power or aera , double. former: TRUE if router_initialize succeeded, FALSE otherwise
...
*/
 double router_initialize(router_info_t *info, router_power_t *router_power, router_area_t *router_area, int Psw, int Pint, int Pst);
...
*/
extern double get_router_power(router_power_t *router, int Psw, int Pst, int Pint);
//extern double get_router_static_power(router_power_t *router);//
