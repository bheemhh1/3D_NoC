...
/* global variables */
GLOBDEF(SIM_router_power_t, SIM_router_power);
GLOBDEF(SIM_router_info_t, SIM_router_info);
GLOBDEF(SIM_router_area_t, SIM_router_area);

int SIM_router_init(SIM_router_info_t *info, SIM_router_power_t *SIM_router_power, SIM_router_area_t *SIM_router_area)
{
	u_int line_width;
	...
	if (info->n_v_class * info->n_v_channel > 1) {
		if ((info->sw_in_arb_model = PARM(sw_in_arb_model)) != 0) {
	...
	if ( info->n_v_channel > 1 && info->n_in > 1) {
		if ((info->vc_in_arb_model = PARM(vc_in_arb_model)) != 0) {
	...
	if(SIM_router_power){
		SIM_router_power_init(info, SIM_router_power);
	}

	if(SIM_router_area){
		SIM_router_area_init(info, SIM_router_area);
	}
