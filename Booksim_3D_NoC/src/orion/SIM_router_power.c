...
int SIM_router_power_init(SIM_router_info_t *info, SIM_router_power_t *router)
{
	...
	if (info->in_buf) {
		...
		/* static power */
		router->I_buf_static = router->in_buf.I_static * (info->n_in-1) * PARM_v_channel;//info->n_v_class * (info->in_share_buf ? 1 : info->n_v_channel);
	}
	...
	if (info->sw_in_arb_model) {
		...
		router->I_sw_arbiter_static = router->sw_in_arb.I_static * info->in_n_switch;
	}
	...
	if (info->sw_out_arb_model) {
		...
		router->I_sw_arbiter_static += router->sw_out_arb.I_static;
	}
	/*static energy*/ 
	...
	if(info->vc_allocator_type == ONE_STAGE_ARB && info->n_v_channel > 1 && info->n_in > 1){
		if (info->vc_out_arb_model)
		{
			...
			router->I_vc_arbiter_static = router->vc_out_arb.I_static *  PARM_v_channel;
		}
		else {
		...
		if (info->vc_in_arb_model && info->vc_out_arb_model)
		{
			...
			router->I_vc_arbiter_static = router->vc_in_arb.I_static *  PARM_v_channel;
		...
		/* static power */
		router->I_vc_arbiter_static = router->vc_select_buf.I_static * PARM_v_channel;
	}
