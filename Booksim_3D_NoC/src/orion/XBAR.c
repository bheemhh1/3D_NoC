...
int XBAR_initialize(XBAR *crsbar)
{
	...
	switch(crsbar->model) {
		case MATRIX_CROSSBAR:
			crsbar->insts = static_cast<int>(crsbar->p_in * crsbar->p_out * pow(crsbar->flit_width, 2)); //flexus-specific
			break;
...
{
        std::cout<<"\n-- XBAR_get_leakage_power: "<<(MUX2_leak_nW * ((XBAR*)crsbar)->insts)<<"\n";
	return MUX2_leak_nW * ((XBAR*)crsbar)->insts;
}
double XBAR_get_internal_power(void *crsbar)
{
	std::cout<<"\n-- XBAR_get_internal_power: "<<(MUX2_int_J * ((XBAR*)crsbar)->tr * ((XBAR*)crsbar)->insts)<<"\n";
	return MUX2_int_J * ((XBAR*)crsbar)->tr * ((XBAR*)crsbar)->insts; 
}

double XBAR_get_switching_power(void *crsbar)
{
  	std::cout<<"\n-- XBAR_get_switching_power: "<<(0.5 * 1.4 * MUX2_load_pF * PARM(VDD_V) * PARM(VDD_V) * ((XBAR*)crsbar)->tr * ((XBAR*)crsbar)->clk)<<"\n";
