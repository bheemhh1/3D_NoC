...
double get_router_power(router_power_t *router, int Psw, int Pint, int Pst)
{	
	if(!(Psw==0 || Psw==1))
	{
	  printf("Psw must be 0 or 1, converted to 1");
	  Psw=1;
	}
	
	if(!(Pst==0 || Pst==1))
	{
	  printf("Pst must be 0 or 1, converted to 1");
	  Pst=1;
	}
	if(!(Pint==0 || Pint==1))
	{
	  printf("Pint must be 0 or 1, converted to 1");
	  Pint=1;
	}
	...
	p_total = Pst*p_leakage + Pint*p_internal+ Psw*p_switching; 
	
	//must be removed
	fprintf(stdout, "Power report of orion INSTSinbuffer:%g\tINSTSoutbuffer:%g\t INSTScrossbar:%g\t INSTSswvc:%g\t INSTSclkctrl:%g\n**** Pleakage:%g\tPinternal:%g\tPswitching:%g\tPtotal:%g\n", 
