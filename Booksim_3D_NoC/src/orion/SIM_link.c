...
double SIM_link_power_report(Router * current_router ){
  ...
  double total_link_power = 0.0;
    for(int output = 0 ; output < ((current_router->NumOutputs())-1) ; output++)
    {
  if(output<4)
  {	
      link_power = 0.0;
      
      link_power = LinkDynamicEnergyPerBitPerMeter(current_router->_orion_link_length[output], PARM_Vdd);
     link_power *= current_router->_orion_link_length[output];
      link_power = link_power * current_router->_orion_link_output_counters[output] * PARM_flit_width / 2 ;
      total_link_power += link_power;
      c=current_router->_orion_link_output_counters[output];
      l3=c+l3;
      
    }
else
{   
     t2=t2+current_router->_orion_link_output_counters[output];
}
   }
//  cout<<"\n l3"<<l3<<endl;
    return total_link_power;
}

double SIM_link_power_report_static(Router * current_router ){
  double link_power=0.0;
  double total_link_power = 0.0;
    for(int output = 0 ; output < ((current_router->NumOutputs())-1) ; output++){
      link_power = 0.0;
      link_power = LinkLeakagePowerPerMeter(current_router->_orion_link_length[output], PARM_Vdd);
      link_power *= current_router->_orion_link_length[output]* PARM_flit_width;
